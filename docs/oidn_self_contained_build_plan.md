...
Status: **IN PROGRESS - partially implemented (2026-06-10)**
...
downloads.** A lighter/prebuilt path may be revisited later (see section 8).

## 0. Implementation status update (2026-06-10)

This document started as a design plan. The following pieces are now implemented
in-tree, and the remaining items are tracked below in sections 8a and 9.

Implemented:
- Top-level OIDN options in `CMakeLists.txt`: `ENABLE_OIDN`, `OIDN_DEVICE`, `OIDN_PREBUILT_DIR`.
- New self-contained OIDN source-build orchestrator in `cmake/oidn.cmake`.
- New escape hatch wiring in `cmake/oidn_prebuilt.cmake`.
- New Ninja resolver/fetch module in `cmake/ninja.cmake`.
- New SYCL dependency modules in `cmake/dpcpp.cmake` and `cmake/level_zero.cmake`.
- `util/CMakeLists.txt` rewired to consume `OpenImageDenoise` target from top-level integration (no local `find_package(OpenImageDenoise)`).
- `cmake/package.cmake` extended with `crt_add_packaged_files(...)` for file-list runtime staging/install.
- `README.md` updated with the new OIDN workflow and options.

Implemented with caveats:
- DPC++ and Level Zero versions are pinned (`v6.2.1`, `v1.29.0`), but archive SHA256 values are still placeholders (user-set cache variables).
- Runtime library staging is wired, but build-order coupling to `oidn_ext` must be hardened (see section 8a).
- OIDN SYCL environment propagation into the OIDN ExternalProject step (`PATH`/`LD_LIBRARY_PATH`) is not yet explicitly added.

Validation status:
- CMake syntax/diagnostics for edited files: clean.
- Full configure/build validation is currently blocked by external SDL2 discovery in this environment (`SDL2Config.cmake` not found).
...
> `CMakeLists.txt` files (only validation, see section 5).
...
work here. This is why the plan commits to a from-source build (section 2).
...
mid-build, so it is not suitable for the SYCL device; see section 7.)
...
absent from every public OIDN binary release (section 1.3).
...
| Toolchain (SYCL only) | Intel DPC++/SYCL + Level Zero, downloaded on demand at **pinned versions** (section 4.3-4.4). Only fetched when `OIDN_DEVICE=SYCL`. |
| Build mechanism | `ExternalProject_Add` (lets OIDN use its own compiler), consistent with the existing `glm`/`rapidjson` pattern |
| Escape hatch | `OIDN_PREBUILT_DIR` (path to an existing OIDN install/build) skips the download+build entirely and just wires the target - useful for CI caching and developers who already have a suitable OIDN (section 4.8). |
...
these must not drift from RPTR (section 4.3-4.4).
...
validates the value and applies the Apple override (sections 4.2 and 6).
...
1. **Escape hatch first:** if `OIDN_PREBUILT_DIR` is set, skip all
   ...
   that directory (section 4.8), then return.
...
6. Record the produced runtime shared libraries in a cache list
   (`OIDN_RUNTIME_LIBRARIES`) for staging/installation (see section 4.5).
...
if (OIDN_PREBUILT_DIR)
    include(cmake/oidn_prebuilt.cmake)   # defines OpenImageDenoise from OIDN_PREBUILT_DIR (section 4.8)
    return()
endif()

# (2) Ninja is required for the OIDN sub-build (see section 4.7).
...
ExternalProject_Add(oidn_ext
    ...
    # (see section 4.3), e.g. via `${CMAKE_COMMAND} -E env` wrappers on the steps.
    CMAKE_ARGS
...
file(GLOB OIDN_RUNTIME_LIBRARIES "${OIDN_INSTALL_DIR}/bin/*${CMAKE_SHARED_LIBRARY_SUFFIX}*")  # + DPC++ runtime, see section 4.5
...
>   prefer a fixed, known list of expected runtime libs (see section 4.5) rather than a
...
>   one that was built (section 4.5).
...
created by `cmake/oidn.cmake`. The `option(...)` moves to the top level (section 4.1).
...
`ExternalProject` as `-DCMAKE_MAKE_PROGRAM=` (section 4.2).
...
| `OIDN_DEVICE` must be one of `SYCL`/`CUDA`/`HIP`/`METAL`/`CPU` | exactly one device per build; reject typos with `FATAL_ERROR` (section 4.2) |
...
| `OIDN_DEVICE=SYCL` ? fetch DPC++ `v6.2.1` + Level Zero `v1.29.0` (pinned) | experimental semaphore interop is version-sensitive (section 4.3-4.4) |
| On macOS, `OIDN_DEVICE` resolves to `METAL` (unless `CPU`) | DPC++/SYCL is not used for the Metal device (section 6) |
...
| `OIDN_PREBUILT_DIR` set ? skip the Git/Ninja/toolchain requirements | nothing is built from source in that case (section 4.8) |
...
- Stage the single device-specific runtime lib that actually got built (section 4.5).
...
These were previously open; they are now settled and reflected in sections 2-7:
...
2. **GPU vendor coverage - one device per build, default SYCL.** A single
  ...
  (sections 4.1, 4.2, 5, and 6). Choosing `HIP` also gives AMD users a path around the
...
3. **DPC++ / Level Zero versions - pinned to RPTR.** The SYCL toolchain is pinned
   ...
   may not be bumped without re-validating interop (section 4.3-4.4).
...
5. **Build time / CI - include the escape hatch + caching.** Added
   ...
   CI build-tree caching note (section 4.8). This is a local path, not a binary download.
6. **Ninja - check then fetch.** `cmake/ninja.cmake` finds Ninja (VS ships it),
   otherwise fetches the small prebuilt binary, otherwise errors (section 4.7).
...
- **Exact DPC++ runtime lib names to stage:** the *versions* are pinned (DPC++
  ...
  `ur_*` libs) on each OS for staging (section 4.5). Cross-check against RPTR's
  `EXT_RUNTIME_LIBRARIES` list.
- **Archive hashes:** fill in the `URL_HASH` values for the pinned DPC++ /
  Level Zero archives (and confirm the Level Zero asset filename) (section 4.3-4.4).
- **Runtime staging dependency ordering:** ensure packaged runtime copy targets
  run only after `oidn_ext` (and SYCL toolchain download targets) have produced
  the libraries, to avoid first-build copy failures.
- **SYCL toolchain env propagation into OIDN sub-build:** verify whether
  explicit `PATH` / `LD_LIBRARY_PATH` injection is required for reliable OIDN
  configure/build across Windows/Linux, and add if needed.
- **GPU-backend warning location:** optional warning for `ENABLE_OIDN=ON` with
  no `ENABLE_DXR`/`ENABLE_VULKAN`/`ENABLE_METAL` is still pending because backend
  options are currently defined in subdirectories.
...
- [x] Move `option(ENABLE_OIDN ...)` to the top-level `CMakeLists.txt` and add the
      `OIDN_DEVICE` enum (default `SYCL`) + `OIDN_PREBUILT_DIR` (section 4.1).
- [x] Rewire `util/CMakeLists.txt` to drop `find_package(OpenImageDenoise)` and
      link the new `OpenImageDenoise` INTERFACE target (section 4.6).
- [x] Add `crt_add_packaged_files` to `cmake/package.cmake` (section 4.5).

**Phase 1 - Generator + toolchain download:**
- [x] Create `cmake/ninja.cmake` (find-or-fetch Ninja) (section 4.7).
- [x] Create `cmake/dpcpp.cmake` (section 4.3) and `cmake/level_zero.cmake` (section 4.4),
...
- [~] Add option validation (section 5).
- [~] Cross-platform gating (section 6); macOS `OIDN_DEVICE_METAL` path (untested for now).
- [x] Update `README.md`: remove the "build OIDN yourself" prerequisite; document
